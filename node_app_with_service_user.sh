...
# Create a service user 'myapp' if it doesn't exist
if ! id -u myapp > /dev/null 2>&1; then
    echo "Creating service user 'myapp'"
    sudo useradd -m -s /bin/bash myapp || error_exit "Failed to create user 'myapp'"
else
...
# Change ownership of the application directory to the service user
sudo chown -R myapp:myapp . || error_exit "Failed to change ownership of the application directory"

# Run the NodeJS application as the 'myapp' user
echo "Installing dependencies and starting the application as 'myapp' user..."
sudo -u myapp -H bash -c "export APP_ENV=dev; export DB_USER=myuser; export DB_PWD=mysecret; export LOG_DIR=$LOG_DIR; cd $(pwd); npm install || exit 1; nohup node server.js > $LOG_DIR/app.log 2>&1 &"

# Wait a few seconds to give the application time to start
sleep 5

# Check if the application is running
APP_PID=$(pgrep -u myapp -f "node server.js")
